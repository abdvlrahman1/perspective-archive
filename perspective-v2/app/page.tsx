...
export default function PerspectiveArchive() {
  ...
  // Changed to local state only - No Supabase
  const [reflections, setReflections] = useState([
    { id: 1, text: "The heart finds rest in remembrance.", replies: [{ text: "A beautiful reminder." }] }
  ]);
  ...
  useEffect(() => {
    ...
    return () => { clearInterval(timer); if(track) track.pause(); };
  }, []);

  // Simplified post function - No Supabase
  const handlePost = (e) => {
    e.preventDefault();
  ...
  return (
    <div className="bg-[#00302e] text-white selection:bg-white selection:text-black min-h-screen font-sans overflow-x-hidden">
        ...
        <div className="max-w-2xl mx-auto">
              ...
              {reflections.map((c) => (
                <motion.div key={c.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative">
                  <div className="max-w-[80%]">
                    <p className="font-serif text-sm md:text-base text-white/50 leading-relaxed tracking-wide">
                      {c.text}
                    </p>
                  </div>

                  {c.replies && c.replies.map((r, idx) => (
                    <div key={idx} className="mt-8 ml-auto md:absolute md:top-full md:left-24 md:max-w-[70%]">
                       <div className="flex items-start space-x-3 opacity-90 md:translate-y-2">
                          <CornerDownRight size={10} className="mt-1 opacity-20 flex-shrink-0 text-white" />
                          <div className="space-y-1">
                            <p className="font-serif italic text-xs md:text-sm text-white/90 leading-snug tracking-tight">
                              {r.text}
                            </p>
          ...
          <form onSubmit={handlePost} className="max-w-md mx-auto relative group border-t border-white/5 pt-12">
            <textarea 
              ...
              className="w-full bg-transparent py-2 font-serif italic focus:outline-none transition-colors placeholder:opacity-10 text-sm md:text-base resize-none pr-12"
              rows={1}
            />
      ...
      {/* FOOTER */}
      <footer className="w-full py-20 flex flex-col items-center justify-center space-y-12 bg-[#00302e]">
          <div className="h-px w-8 bg-white/10"></div>
          <div className="flex items-center space-x-10 md:space-x-20">
            <a href="https://wa.me/254797620060" className="opacity-30 hover:opacity-100 transition-all"><MessageCircle size={20} /></a>
            <a href="https://substack.com/@abdvlrahman" className="opacity-30 hover:opacity-100 transition-all"><ExternalLink size={20} /></a>
            <a href="mailto:abdirahmanvomar1@gmail.com" className="opacity-30 hover:opacity-100 transition-all"><Mail size={20} /></a>
          </div>
