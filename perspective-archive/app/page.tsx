...
import { Clock, Mail, ExternalLink, MessageCircle, Download, Send, CornerDownRight } from "lucide-react";

// 1. POEM COLLECTION - FULLY RESTORED
const POEMS = [
  {
    id: 1,
    ...
    content: (
      <>
        ...
        <p className="text-white font-normal italic pt-6 text-sm md:text-base">
          you know what,<br/>
...
// 2. LIBRARY DATA
const BOOKS = [
  { title: "Taj al-Arus", author: "Ibn Ata'illah", image: "/taj-al-arus.png", link: "/book.pdf" },
...
export default function PerspectiveArchive() {
  ...
  const [reflections, setReflections] = useState([
    { 
      id: 1, 
      text: "The heart finds rest in remembrance.", 
      date: "25.12",
      replies: [{ id: 101, text: "A beautiful reminder for us all.", date: "25.12" }]
    }
  ]);
  ...
  return (
    <div className="bg-[#00302e] text-white selection:bg-white selection:text-black min-h-screen font-sans overflow-x-hidden">
      {/* TEXTURE OVERLAY */}
      ...
      <nav className="fixed top-0 w-full p-6 md:p-10 flex justify-between items-start z-50 mix-blend-difference">
        <div className="flex flex-col space-y-1">
          <span className="text-[9px] md:text-[10px] tracking-[0.5em] uppercase font-bold text-white">Abdirahman Omar</span>
        ...
        <button onClick={() => {
          ...
          <span className="text-[8px] tracking-[0.3em] uppercase hidden md:inline">{isPlaying ? "Atmosphere: On" : "Muted"}</span>
        </button>
      </nav>
      ...
      <section className="min-h-screen px-6 md:px-10 flex flex-col items-center justify-center text-center">
        <div className="max-w-lg space-y-8 md:space-y-12">
          ...
          <button onClick={() => setPoemIndex((prev) => (prev + 1) % POEMS.length)} className="group flex flex-col items-center space-y-4 pt-10 opacity-30 hover:opacity-100">
      ...
      <section className="bg-[#FAF9F6] text-black py-32 md:py-48 px-6">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-24">
            {BOOKS.map((book, i) => (
              <motion.div key={i} whileHover={{ y: -10 }} className="flex flex-col items-center space-y-6 md:space-y-8 group">
                <div className="relative w-40 h-60 md:w-48 md:h-72 shadow-[15px_15px_40px_-10px_rgba(0,0,0,0.15)] group-hover:shadow-[25px_25px_60px_-10px_rgba(0,0,0,0.25)] transition-all duration-700">
                  <img src={book.image} alt={book.title} className="w-full h-full object-cover grayscale-[30%] group-hover:grayscale-0 transition-all duration-700" />
                  <div className="absolute inset-0 bg-gradient-to-r from-black/10 to-transparent pointer-events-none"></div>
                </div>
                <div className="text-center space-y-2">
                  <h3 className="font-serif text-xl md:text-2xl italic">{book.title}</h3>
                  <p className="text-[9px] uppercase tracking-widest opacity-40">{book.author}</p>
                  <a href={book.link} download className="inline-block pt-4 opacity-10 group-hover:opacity-100 transition-all">
        ...
        <div className="max-w-2xl mx-auto">
              ...
              {reflections.map((c) => (
                <motion.div key={c.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative">
                  <div className="max-w-[85%]">
                    <p className="font-serif text-sm md:text-base text-white/60 leading-relaxed tracking-wide">{c.text}</p>
                    <span className="text-[7px] tracking-[0.3em] uppercase opacity-20 block mt-3">{c.date}</span>
                  </div>
                  {c.replies.map((r) => (
                    <div key={r.id} className="mt-6 md:mt-2 ml-auto md:absolute md:top-full md:-right-8 md:max-w-[70%]">
                       <div className="flex items-start space-x-3 opacity-80 md:translate-y-2">
                          <CornerDownRight size={10} className="mt-1 opacity-20 flex-shrink-0" />
                          <div className="space-y-1">
                            <p className="font-serif italic text-xs md:text-sm text-white/80 leading-snug tracking-tight">{r.text}</p>
          ...
          <form onSubmit={handlePost} className="max-w-md mx-auto relative group border-t border-white/5 pt-12">
            <textarea 
              ...
              className="w-full bg-transparent py-2 font-serif italic focus:outline-none placeholder:opacity-10 text-sm md:text-base resize-none pr-12"
              rows={1}
            />
            <button type="submit" className="absolute right-0 bottom-3 opacity-20 group-hover:opacity-100 transition-opacity p-2">
      ...
      <footer className="w-full py-24 flex flex-col items-center justify-center space-y-10 bg-[#00302e] border-t border-white/5">
          <div className="flex items-center space-x-12">
            <a href="https://wa.me/254797620060" target="_blank" rel="noopener noreferrer" className="opacity-20 hover:opacity-100 transition-all duration-500 hover:scale-110">
              <MessageCircle size={18} strokeWidth={1.5} />
            </a>
            <a href="https://substack.com/@abdvlrahman" target="_blank" rel="noopener noreferrer" className="opacity-20 hover:opacity-100 transition-all duration-500 hover:scale-110">
              <ExternalLink size={18} strokeWidth={1.5} />
            </a>
            <a href="mailto:abdirahmanvomar1@gmail.com" className="opacity-20 hover:opacity-100 transition-all duration-500 hover:scale-110">
